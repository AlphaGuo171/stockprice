...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            cost_line = go.Scatter(x=df['日期'], y=[cost_price] * len(df['日期']), mode='lines', line=dict(color='red', dash='dash'))
            fig.add_trace(cost_line)
            
            fig.update_layout(title=stock_code + '成本数据与持股成本对比', xaxis_title='日期', yaxis_title='成本')
